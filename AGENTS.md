...
Distribution packages are the supported install path; there is no tarball/script installer. CI (`.github/workflows/release.yml`) builds RPM and DEB for `x86_64`/`aarch64` on tag pushes; the pacman package is built locally by users from `packaging/PKGBUILD`.
...
- **pacman:** `packaging/PKGBUILD` plus `packaging/cli-assistant.install` (post-install hint). `arch=('x86_64' 'aarch64')`, built locally with `makepkg`. **CI does not build it:** linking fails inside `archlinux:base-devel` (unresolved symbols of ring's assembly and bundled SQLite, reproduced with rust 1.95/1.98, system lld/rust-lld, and Arch's CFLAGS/LDFLAGS — but never outside that container), and local `makepkg` is the Arch convention anyway. The PKGBUILD pins `-C linker=cc -C linker-features=-lld` and `LIBSQLITE3_SYS_USE_PKG_CONFIG=1` as defensive measures; Fedora and Debian keep the bundled (static) SQLite.
