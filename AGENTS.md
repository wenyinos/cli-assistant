...
- **Shell integration:** `c shell --enable-interactive` appends a marked block to `~/.bashrc` or `~/.zshrc` (picked from `$SHELL`) that binds Ctrl+G to `c chat --interactive`; `--disable-interactive` strips the block from both. Keep the `command -v c` lookup rather than an absolute path, and do not go back to `~/.bashrc.d` — most distributions never source it.
- **Version bumps:** `workspace.package.version` (currently 0.9.1). A release also touches `packaging/cli-assistant.spec` (Version + changelog), `debian/changelog`, `packaging/PKGBUILD` (pkgver), and the man pages' `.TH` line — grep for the old version string before cutting a tag.
...
Distribution packages are the supported install path; there is no tarball/script installer. CI (`.github/workflows/release.yml`) builds RPM and DEB for `x86_64`/`aarch64` plus pacman for `x86_64` on tag pushes; Arch Linux ARM builds the aarch64 package locally from `packaging/PKGBUILD`.
...
- **pacman:** `packaging/PKGBUILD` plus `packaging/cli-assistant.install` (post-install hint). `arch=('x86_64' 'aarch64')`. **Do not remove the LTO override in `build()`:** Arch's makepkg enables LTO by default and injects `-flto=auto` into CFLAGS/CXXFLAGS/LDFLAGS, so the C objects of sqlx's bundled SQLite and ring carry LTO bitcode that the final rustc link cannot resolve (`undefined symbol: sqlite3_*`, `ring_core_*`). The recipe strips those flags and pins `-C lto=off`; reproduce inside an `archlinux:base-devel` container (`podman run --rm archlinux:latest … makepkg`) when in doubt.
